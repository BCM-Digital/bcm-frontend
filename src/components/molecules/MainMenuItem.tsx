...
function MainMenuItem({ menuItem }: MainMenuItemProps): ReactElement | null {
	...
	const linkClasses = clsx(
		'font-heading group text-[2.5rem] text-primary font-extrabold uppercase tracking-wider',
					...
					<Fragment>
						{
							menuItem.label && (
								...
								>
									...
									<span
										className="relative ml-auto block h-4 w-4 flex-none text-primary tablet-landscape:h-2.5 tablet-landscape:w-2.5">
							<span
								className="absolute inset-0 m-auto block h-0.5 -rotate-90 bg-current transition-all duration-200 group-aria-expanded:-rotate-180"></span>
							<span
								className="absolute inset-0 m-auto block h-0.5 rotate-180 bg-current transition-all duration-200 group-aria-expanded:rotate-180"></span>
						</span>
								</button>
							)
						}
